...
public class Z_InboundDeliveryDocumentsPostBAPI extends BAPI {
	...
	public Z_InboundDeliveryDocumentsPostBAPI(User u, GregorianCalendar lu) throws Exception {
		super(u, lu);
		// TODO Auto-generated constructor stub
		int docCount = Integer.parseInt(this._user.eval("<<size transaction.Z_POrders>>"));
	...
	public Z_InboundDeliveryDocumentsPostBAPI(User u, GregorianCalendar lu, Order ord, OrderItem item, Z_DocumentLink _doclink) throws Exception {
		super(u, lu);
		inboundOrd = ord;
		inboundOrdItem = item;
		doclink = _doclink;
	...
	/*
	 * custom setParameters() method for Z_InboundDeliveryDocumentsPostBAPI to save the custom values to the IT_RECEIPT.
	 * */
			...
			for (int j = 0; j < docCount; j++) {
				...
				for (int i = 0; i < itemCount; i++) {
					//if (_inboundDocs.get(j).getItems()[i].Z_IsSelected.length()>0) {
						...
						setValue(receiptTbl, "VBELN", _inboundDocs.get(j).getOrderID());
						...
						setValue(receiptTbl, "POSNR", _inboundDocs.get(j).getItems()[i].getZ_IDLineItemNo());
						...
						setValue(receiptTbl, "GRUND_UN", _inboundDocs.get(j).getItems()[i].getZ_GenMovementReason());
						...
						setValue(receiptTbl, "BDATE", _user.eval("<<transaction.Z_BookingDate format=\"%Y%m%d\">>"));
						//setValue(receiptTbl, "LFIMG", _inboundDocs.get(j).getItems()[i].getQuantity());
						if(_inboundDocs.get(j).getItems()[i].getZ_BlockedQuantity().length()>0 && Double.parseDouble(_inboundDocs.get(j).getItems()[i].getZ_BlockedQuantity())>0){
							Double qty = Double.parseDouble(_inboundDocs.get(j).getItems()[i].getQuantity()) - Double.parseDouble(_inboundDocs.get(j).getItems()[i].getZ_BlockedQuantity());
						...
						if(_inboundDocs.get(j).getItems()[i].Z_IsSelected.length()>0){
							setValue(receiptTbl, "STATUS", "X");
						}
						if (_inboundDocs.get(j).getItems()[i].Z_IsBlocked.equalsIgnoreCase("X")) {
							//receiptTbl.appendRow();
							//setValue(receiptTbl, "VBELN", _inboundDocs.get(j).getOrderID());
							setValue(receiptTbl, "LFIMG_B", _inboundDocs.get(j).getItems()[i].getZ_BlockedQuantity());
		...
		try {
			...
			for (int i = 0; i < numHdrRecs; ++i) {
				...
				for (int j = 0; j < _inboundDocs.size(); j++) {
					if (_Inboundtbl.getString("VBELN").equalsIgnoreCase(_inboundDocs.get(j).getOrderID())) {
